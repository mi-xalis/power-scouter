...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS workout_sets
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  ...
                  reps INTEGER,
                  set_number INTEGER,
                  rpe_rating INTEGER,      -- renamed from rp_rating
                  FOREIGN KEY(user_id) REFERENCES users(id),
...
with st.sidebar:
    st.header("It's Over 9000!")
    st.image("assets/over9k.png", use_container_width=True)
...
with tab1:
    ...
    default_index = 0
    if "new_session_created" in st.session_state and st.session_state.new_session_created:
        if not sessions_df.empty:
            default_index = 1
    ...
    if selected_session_id != 0:
        ...
        with col2:
            if st.button("🗑️ Delete Workout", type="primary"):
                st.session_state.confirm_delete = selected_session_id
        if "confirm_delete" in st.session_state and st.session_state.confirm_delete == selected_session_id:
    ...
    if exercises_df.empty:
        ...
        # When exercise changes, initialize its set list if needed
        ...
        # Display existing sets for this exercise
        ...
        with col1:
            weight = st.number_input("Weight (kg)", min_value=0.0, step=2.5, key="new_weight")
        with col2:
            reps = st.number_input("Reps", min_value=1, step=1, key="new_reps")
        with col3:
            rpe = st.slider("RPE", 0, 10, 5, key="new_rpe")  # renamed
        with col4:
            if st.button("➕ Add Set"):
                if weight > 0 and reps > 0:
                    sets_list.append((weight, reps, rpe))
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("✅ Finish This Exercise"):
                st.session_state.current_exercise = None
                st.rerun()
        with col2:
            if sets_list and st.button("🗑️ Clear Sets for This Exercise"):
                st.session_state.workout_log[selected_exercise_id] = []
                st.rerun()
        ...
        if sessions_df.empty:
            ...
            if session_sets.empty:
                ...
                for idx, row in session_sets.iterrows():
                    col1, col2 = st.columns([0.1, 0.9])
                    with col1:
                        if st.checkbox("", key=f"del_session_{row['id']}"):
                            selected_indices.append(row['id'])
                    with col2:
